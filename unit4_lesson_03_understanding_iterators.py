...
Python builtins like sequences (strings, lists, tuples), sets and dicts are iterable (ie) you can call iter(obj)
 on them
...
def test_iter_on_iterator():
    ...
    # iter on iterator object returns the same object by convention. This facilitates functions to work
    #  on either iterables
...
def test_find_builtins_that_work_on_iterables():
    funcs = ['list',"tuple()","set()","dict()","str()","max()","min()","enumerate()","iter()","len()"]
    assert 10 == len(funcs)
...
def my_map(func, iterable):
    # assert True == hasattr(iterable, "__next__")
    k = iter(iterable)
    r = []
    z = 0
    try:
