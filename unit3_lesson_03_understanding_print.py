...
print_debugging = '''
As the name suggests, you add additional print statements as required
 into the buggy function to be
clear about what intermediate states are, when the test fail. You need to 
figure out what
additional state you want to see etc.

Then you infer what went wrong based on the print output.

This is by far the most common debugging method used and you can go quite
 far with this for  
...
def buggy_contains(current, new):
    for mail in current:
        if abs(ord(mail[0]) - ord(new[0])) == 0 and abs(ord(mail[2]) - ord(new[2])) == 32:
            return True
        if abs(ord(mail[0]) - ord(new[0])) == 32 and abs(ord(mail[2]) - ord(new[2])) == 32:
            return True

        if abs(ord(mail[0])- ord(new[0]))==32  and abs(ord(mail[2])- ord(new[2]))==0:
            return True
        if mail == new:
            return True
