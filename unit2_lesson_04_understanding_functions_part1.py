...
language and makes it easy to expose elegant and
 usable apis.
...
#This makes it easy to access help from console or build specialized help commands
#  like help.
def test_function_documentation():
    assert """returns 10""" == demo1.__doc__
    assert 'me' == demo2.__doc__
   ...
   # pass # replace this calls to min and max
    ...
    for i in range(len(input)):
        ...
        if (len(input[i]) < min):
            min = len(input[i])
            ret[1] = i
    return (input[ret[1]],input[ret[0]])
