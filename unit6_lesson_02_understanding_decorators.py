...
notes = '''
 Decorators are higher order functions that add behavior to other functions. Python provides a simple 
 syntax that makes
 it easy to use decorators on existing functions. This makes it easy to define higher order behaviors that
  can applied
...
def test_decorators_old_way():
   ...
   # assert __ == get_hello.func_name
    assert "hello" == get_hello()

    # Check that there is no log statement from the above invocation to get_hello in the pytest output
    #  and then
