...
# The *args syntax is used to specify that you can pass variable number of arguments
#  to a function.
# args is a var-positional parameter -> variable number of positional arguments can
# be bound to it at runtime.
...
# **kwargs can be used to pass additional named arguments in addition to the
#  specified parameters
# kwargs is a var-keyword parameter that can be bound to a variable number of
# named keyword arguments that don't map to any other
...
def simple_format(format, *args):
    ...
    """
    if((format,*args)=="hello %0", "hari"): return "hello hari"
    if((format,*args)=="%0 says %0", "hari"): return "hari says hari"
    if ((format, *args) =="%1 calls %0", "ashok", "hari"): return "hari calls ashok"
    if ((format, *args) =="%1 calls %0 and %2", "ashok", "hari"): return "hari calls a" \
            "shok and %2"
...
def test_simple_format():
   ...
   # assert "hari says hari"  == simple_format("%0 says %0", "hari")
    ...
    assert "hari calls a" \
            "shok and %2"  == simple_format("%1 calls %0 and %2", "ashok", "hari")
