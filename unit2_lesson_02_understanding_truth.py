...
def test_truth_none():
    value = None
    result = "not-set"
    #is None treated as true or false? false
    if value:
        result = "true"
    else:
        result = "false"
...
def test_truth_values():
    assert "empty string"," is treated as true" == truth_test("", "empty string")
    assert "empty tuple"," is treated as true" == truth_test((), "empty tuple")
    assert "empty list","is treated as true" == truth_test([], "empty list")
    assert "empty dict","is treated as true" == truth_test({}, "empty dict")
    assert "empty set","is treated as true" == truth_test(set(), "empty set")
    assert "white space","is treated as true"== truth_test(" ", "white space")
    assert "0","is treated as true" == truth_test(0, "0")
